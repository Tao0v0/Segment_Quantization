...
# 检查第一个任务是否成功完成
if [ $? -eq 0 ]; then
  echo "train_rgb_day.sh completed successfully."
else
  echo "train_rgb_day.sh failed."
  exit 1
fi

# 运行第一个任务
sh train_rgb_night.sh

# 检查第一个任务是否成功完成
if [ $? -eq 0 ]; then
  echo "train_rgb_night.sh completed successfully."
else
  echo "train_rgb_night.sh failed."
  exit 1
fi
