...
class Attention(nn.Module):
    def __init__(self, dim, head, sr_ratio): # head 为 1
        super().__init__()
        self.head = head
    ...
    def forward(self, x: Tensor, H, W, metric: Tensor=None) -> Tensor:
        ...
        if self.sr_ratio > 1:       # sr_ration是8,作用是降低计算量 每个阶段 8/4/2/1
...
class PatchEmbed(nn.Module):         # 把（B,C,H,W）变成（B,HW,C） 变成了token的形式
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.proj(x)
        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2)    # x的shape变成(B, HW, C)
        x = self.norm(x)
        return x, H, W

class PatchEmbedParallel(nn.Module):  # 不会用
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, H, W, metric: Tensor=None) -> Tensor:
        x = x + self.drop_path(self.attn(self.norm1(x), H, W, metric))      # metric一直为None
...
cmnext_settings = {
    ...
    'B2': [[64, 128, 320, 512], [3, 4, 6, 3]],      # 每个stage的维度，每个stage的block数量
...
class CMNeXt(nn.Module):
    def __init__(self, model_name: str = 'B0', modals: list = ['rgb', 'depth', 'event', 'lidar'], with_events=False) -> None:
        ...
        cur = 0
        self.block1 = nn.ModuleList([Block(embed_dims[0], 1, 8, dpr[cur+i]) for i in range(depths[0])]) # dpr参数推理时不管用，采用eval()模式
    ...
    def forward(self, x: list, x_ext: list=None, metric: Tensor=None) -> list:
        x_cam = x[0]    # 取第一个模态作为主模态，x是来自不同模态的输入列表，第一个模态是RGB图像
        metric_ = None  # metric 是给注意力「打方向盘」的外部引导信号。传了它之后，Block 里计算 Q 的输入不再用原始特征 x，而是用你提供的 metric
        if self.num_modals > 0:
            x_ext = x[1:]
        B = x_cam.shape[0]
        outs = []
        if self.with_events:
            outs_event = []
        # stage 1
        x_cam, H, W = self.patch_embed1(x_cam) # 把(B,C,H,W)变成(B,HW,C)，把图片token化
