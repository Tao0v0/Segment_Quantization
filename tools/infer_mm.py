...
class SemSeg:   # 负责模型构建、预处理、推理、后处理。
    def __init__(self, cfg) -> None:
        # inference device cuda or cpu
        self.device = torch.device(cfg['DEVICE'])

        # get dataset classes' colors and labels
        self.palette = eval(cfg['DATASET']['NAME']).PALETTE     # 根据数据集类（比如 DSEC、KITTI360、DELIVER），获取颜色映射
        self.labels = eval(cfg['DATASET']['NAME']).CLASSES      # 根据数据集类（比如 DSEC、KITTI360、DELIVER），获取类别名
        ...
        msg = self.model.load_state_dict(torch.load(cfg['EVAL']['MODEL_PATH'], map_location='cpu'))         # 加载训练好的权重
        print(msg)
        self.model = self.model.to(self.device)
        self.model.eval()                       # 放到指定设备，设为eval
...
if __name__ == '__main__':      #解析参数、加载配置、循环跑数据
    ...
    semseg = SemSeg(cfg)        # cfg是一个字典文件，里面存放了推理所需要的所有参数
